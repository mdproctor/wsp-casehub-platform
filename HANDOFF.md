...
Closed three XS/S CloudEvent conformance issues (#107, #108, #109) on branch `issue-107-cloudevent-cleanup`. Key finding: GE rule 3 (`.exceptionally()` for CloudEvent dispatch) must NOT be applied in Kafka/AMQP processors that chain `.thenCompose(message.ack())` — it would silently ack messages on dispatch failure. Updated GE-20260621-629712 with the exception; filed protocol PP-20260623-a0fe15. Squashed 9→6 commits, resolved CLAUDE.md conflict (ACL commit had landed on origin/main between branch start and close).
