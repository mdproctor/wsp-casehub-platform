...
The binding contract: when eidos selects an agent with `modelFamily="claude"`, the model selection layer queries `ModelRegistry` with `family="claude"` (and optionally tier/capability filters) to find available models. The `family` filter on `ModelQuery` (D3 revision) provides the typed predicate.
