...
|-----------|------|---------------|-----|
| **rest-spring-generator** | Maven plugin | ~97 `@Path` types across all repos (after filtering: excludes `*.rest.generated.*`, `@RegisterRestClient` interfaces, SSE endpoints) | JAX-RS is framework-specific — no neutral equivalent |
...
**Scope:** Only hand-written, concrete JAX-RS resource classes. The generate and verify goals apply the same input filters:

| Filter | Mechanism | Rationale |
|--------|-----------|-----------|
| Skip `*.rest.generated.*` packages | Package name in Jandex | graphql-generator output — handled by graphql-spring-generator (D8) |
| Skip interfaces | `ClassInfo.isInterface()` | JAX-RS resources are always concrete classes. Interfaces with `@Path` are MicroProfile REST Client definitions (outbound HTTP clients, e.g. `ScimClient`, `Mem0Client`, `GraphitiClient`), not inbound resources |
| Skip `@RegisterRestClient` types | Jandex annotation check | MicroProfile REST Client interfaces — these need Spring `WebClient` equivalents (hand-written), not generated `@RestController` classes |
| Skip SSE endpoints | `@Produces(SERVER_SENT_EVENTS)` or `Multi<T>` return | Category C — framework-specific streaming, hand-written in `-spring` modules |
...
**Streaming endpoint exclusion:** JAX-RS resources returning `Multi<T>` with `@Produces(MediaType.SERVER_SENT_EVENTS)` (e.g. `CaseStreamResource`, `ExecutionStateResource` in the engine) are **excluded from generation scope**. These are Category C resources — the SSE transport and reactive broadcasting infrastructure (`BroadcastProcessor`, `@ObservesAsync`) are inherently framework-specific with no extractable framework-neutral core. Both the generate and verify goals skip them.

Detection: methods annotated with `@Produces` where the value contains `MediaType.SERVER_SENT_EVENTS`, or methods returning `io.smallrye.mutiny.Multi` or `org.reactivestreams.Publisher`.

Spring SSE equivalents are hand-written in the `-spring` module using `SseEmitter` + `ApplicationEventPublisher`, sharing snapshot composition logic from core POJOs where applicable. Tracked as part of each consumer's Spring migration, not this generator epic.
...
**Verify goal:** Every in-scope `@Path`-annotated class in the Quarkus Jandex (after applying the input filters above) must have a corresponding `@RestController` in generated + hand-written Spring sources. Hand-written `@RestController` classes satisfy the check equally — this is how Category C modules (e.g. `streams-webhook`) pass verification: their `-spring` modules contain hand-written controllers that match the Quarkus resource. Unmatched `ContainerResponseFilter` → `ResponseBodyAdvice` mappings are flagged for manual review.
